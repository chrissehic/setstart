...
import { COMPANY_STAGES, WorkflowData } from "@/types/workflow";
import {
  ResizableHandle,
  ResizablePanel,
  ResizablePanelGroup,
...
const cardClass =
  "flex h-full overflow-scroll! flex-col bg-accent w-full rounded-2xl group border pb-26 bg-card";
      ...
      >
        ...
        >
          <ResizablePanel
            defaultSize={40}
            minSize={20}
            className={cn("relative", cardClass)}
                    ...
                    {item.value === "growth-stage" && (
                      <section className={sectionClass}>
                        <Tooltip>
                          <TooltipTrigger>
                            <h4 className="scroll-m-20 text-xl font-medium inline-flex justify-center items-center gap-2">
                              {item.title} <Info className="size-4" />
                            </h4>
                          </TooltipTrigger>
                          <TooltipContent>
                            <Link
                        ...
                        <StagePreview
                          stage={stageObj}
                          isLast={
                            COMPANY_STAGES.length === stageObj.stageNumber
                          }
                        />
                      </section>
                    )}
                      ...
                      <section className={sectionClass}>
                        <div className="flex flex-col justify-start gap-2 w-full">
                          ...
                          <div className="flex flex-col items-center justify-center text-center">
                            ...
                            <p className="mt-2 text-sm text-muted-foreground max-w-lg text-wrap">
                              You haven’t described your offering yet. Outline
                              the product, service, or combination you deliver
                              to customers and align your team behind it.
                            </p>
                            <Button className="mt-2" variant={"link"}>
                              Set it up
                              <ArrowRight className="size-4" />
            ...
            >
              <header className="w-full sticky top-0 p-4 flex flex-row justify-between items-center z-30 backdrop-blur-md border-b border-accent">
                ...
                <div className="flex-1 text-center justify-center items-center">
                  <h4 className="scroll-m-20 text-sm font-medium capitalize">
                    {tabActive}
                  </h4>
                </div>
